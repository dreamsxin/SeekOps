...
5. 在“请求账本”查看趋势、费用、错误和明细，在“告警中心”处理账号、余额、配额和错误率异常。
6. 在“操作记录”追查管理变更，在“设置”检查恢复条件并下载 SQLite 与主密钥完整备份。
...
- 按模型维护带生效时间的价格版本；请求绑定实际采用的版本，缺价或缺少 usage 时明确标记为无法估算。
- 上游余额轮询、当前状态和历史快照。
- SQLite WAL 持久化；上游及租户凭据 AES-256-GCM 加密。
- 历史明文自动迁移、本地主密钥轮换、错误密钥启动保护和备份恢复说明。
- 控制台展示 OpenAI/Anthropic Base URL、平台 API Key 和请求示例。
- 账号 API 连通性测试：模型列表和最小 Chat 请求，可查看状态、延迟和响应摘要，并将 `/models` 返回结果同步为账号支持模型。
- 代理故障转移：网络错误、429、500/502/503/504 最多切换一个健康账号，账本保存最终账号和尝试次数。
- SQLite 告警中心：账号检测失败、低余额、租户配额 80%/100% 和近期错误率告警，支持确认、静默、自动恢复及阈值配置。
- SQLite 操作记录：追踪账号、租户密钥、配额、安全凭据、价格、告警和备份操作，不保存明文密钥。
- 控制台一致性备份：下载包含 SQLite、备份清单和本地主密钥的 ZIP，并在下载前检查数据库结构与密钥匹配状态。
- 会话亲和路由：按租户密钥隔离的会话指纹优先复用同一上游账号；亲和账号不健康或重试失败时回到健康账号池并立即重绑。
- 缓存效果 A/B：稳定会话按固定比例分亲和组与对照组，账本按上游实际返回的缓存命中/未命中 Token、延迟、成功率和回退次数汇总。
...
用户价值：在不保存 Prompt、输出或 `reasoning_content` 的前提下，让同一会话有机会复用同一上游账号，并用真实缓存 Token 判断是否有效。

验收标准：

- 优先读取 `X-Proxy-Session-ID`，兼容 `X-Conversation-ID`；Chat/Anthropic 没有会话头时，仅对稳定的系统消息、工具定义和首个用户消息生成哈希指纹。
- 指纹包含租户密钥范围，不跨租户共享；只保留内存路由关系，默认 TTL 24 小时、最多 100000 条。
- 默认 90% 亲和组、10% 对照组，可通过 `SESSION_AFFINITY_PERCENT` 调整；无稳定会话标识的请求单独归为无会话组。
- 亲和账号不可用或遇到可重试故障时，必须从健康账号池切换；成功切换后把会话亲和迁移到新账号。
- 请求账本和 `/admin/usage/summary` 暴露分组、复用、回退、缓存命中率和平均延迟；不把同账号视为缓存命中保证。
