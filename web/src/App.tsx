...
function KeyModal({ keyItem, onClose, onSave, onRotate, onRevoke }: { keyItem: VirtualKey; onClose: () => void; onSave: (body: VirtualKeyInput) => Promise<VirtualKey>; onRotate: () => Promise<VirtualKey>; onRevoke: () => Promise<void> }) {
  ...
  const [busy, setBusy] = useState("");
  const [error, setError] = useState("");
  const payload = (): VirtualKeyInput => ({ name: name.trim(), tenant_id: tenant.trim(), enabled: current.id === "vk-default" ? true : enabled, allowed_models: allowedModels.split(",").map((item) => item.trim()).filter(Boolean), quota: { requests_per_minute: numberOrZero(rpm), concurrent_requests: numberOrZero(concurrent), daily_tokens: numberOrZero(tokens), daily_cost_cny: numberOrZero(cost), monthly_cost_cny: numberOrZero(monthlyCost) } });
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    setBusy("save");
    setError("");
  ...
  return <div className="modal-backdrop"><div className="modal key-modal"><div className="modal-head"><div><h2>管理租户密钥</h2><p>{current.id} · 创建于 {formatTime(current.created_at)}</p></div><button className="icon-button" title="关闭" onClick={onClose}><X size={19} /></button></div><div className="key-modal-body"><section className="key-credential"><div className="section-label">API Key</div><div className="key-credential-value"><code>{current.secret_available ? (revealed ? current.secret : `${current.prefix}••••••••`) : "此密钥由旧版本创建，当前无法恢复"}</code><button className="icon-button small" title={revealed ? "隐藏密钥" : "查看密钥"} disabled={!current.secret_available} onClick={() => setRevealed((value) => !value)}>{revealed ? <EyeOff size={15} /> : <Eye size={15} />}</button><button className="icon-button small" title="复制密钥" disabled={!current.secret_available} onClick={async () => { await navigator.clipboard.writeText(current.secret); setCopied(true); window.setTimeout(() => setCopied(false), 1600); }}>{copied ? <Check size={15} /> : <Clipboard size={15} />}</button></div>{!current.secret_available && current.id !== "vk-default" && <p className="settings-note">轮换后会生成可查看的新密钥，旧密钥立即失效。</p>}</section><section><div className="section-label">当前用量</div><div className="key-usage-grid"><div><span>本分钟请求</span><strong>{formatNumber(current.usage.requests_this_minute)}</strong></div><div><span>处理中</span><strong>{formatNumber(current.usage.active_requests)}</strong></div><div><span>今日 Token</span><strong>{formatCompact(current.usage.daily_tokens)}</strong></div><div><span>今日费用</span><strong>¥ {current.usage.daily_cost_cny.toFixed(4)}</strong></div><div><span>本月费用</span><strong>¥ {current.usage.monthly_cost_cny.toFixed(4)}</strong></div></div></section><form onSubmit={submit} className="create-form key-form"><div className="section-label">租户与配额</div><div className="form-grid"><label>名称<input value={name} onChange={(e) => setName(e.target.value)} required /></label><label>租户 ID<input value={tenant} onChange={(e) => setTenant(e.target.value)} required /></label><label>每分钟请求<input type="number" min="0" value={rpm} onChange={(e) => setRpm(e.target.value)} placeholder="不限" /></label><label>最大并发<input type="number" min="0" value={concurrent} onChange={(e) => setConcurrent(e.target.value)} placeholder="不限" /></label><label>每日 Token<input type="number" min="0" value={tokens} onChange={(e) => setTokens(e.target.value)} placeholder="不限" /></label><label>每日费用（元）<input type="number" min="0" step="0.01" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="不限" /></label><label>每月预算（元）<input type="number" min="0" step="0.01" value={monthlyCost} onChange={(e) => setMonthlyCost(e.target.value)} placeholder="不限，达到后拒绝请求" /></label><label>可用模型<input value={allowedModels} onChange={(e) => setAllowedModels(e.target.value)} placeholder="留空表示全部模型" /></label></div><label className={`check-row ${current.id === "vk-default" ? "disabled" : ""}`}><input type="checkbox" checked={enabled} disabled={current.id === "vk-default"} onChange={(e) => setEnabled(e.target.checked)} />启用密钥{current.id === "vk-default" && "（平台默认 Key 始终启用）"}</label>{error && <p className="form-error">{error}</p>}<div className="modal-actions key-modal-actions">{current.id !== "vk-default" && <><button type="button" className="secondary" disabled={Boolean(busy)} onClick={rotate}><RefreshCw size={15} />{busy === "rotate" ? "正在轮换" : "轮换密钥"}</button><button type="button" className="danger-action" disabled={Boolean(busy)} onClick={async () => { if (!confirm("撤销后客户端将立即无法使用该密钥。继续吗？")) return; setBusy("revoke"); setError(""); try { await onRevoke(); } catch (err) { setError(err instanceof Error ? err.message : "撤销密钥失败"); setBusy(""); } }}>撤销</button></>}<span className="modal-action-spacer" /><button type="button" className="secondary" onClick={onClose}>关闭</button><button className="primary" disabled={Boolean(busy)}>{busy === "save" ? "正在保存" : "保存配置"}</button></div></form></div></div></div>;
}
...
function CreateKeyModal({ secret, onClose, onCreate }: { secret: string; onClose: () => void; onCreate: (body: VirtualKeyInput) => Promise<void> }) {
  const [name, setName] = useState(""); const [tenant, setTenant] = useState(""); const [rpm, setRpm] = useState(""); const [concurrent, setConcurrent] = useState(""); const [tokens, setTokens] = useState(""); const [cost, setCost] = useState(""); const [monthlyCost, setMonthlyCost] = useState(""); const [allowedModels, setAllowedModels] = useState(""); const [busy, setBusy] = useState(false); const [copied, setCopied] = useState(false);
  const submit = async (e: FormEvent) => { e.preventDefault(); setBusy(true); try { await onCreate({ name, tenant_id: tenant, enabled: true, allowed_models: allowedModels.split(",").map((item) => item.trim()).filter(Boolean), quota: { requests_per_minute: numberOrZero(rpm), concurrent_requests: numberOrZero(concurrent), daily_tokens: numberOrZero(tokens), daily_cost_cny: numberOrZero(cost), monthly_cost_cny: numberOrZero(monthlyCost) } }); } finally { setBusy(false); } };
  return <div className="modal-backdrop"><div className="modal"><div className="modal-head"><div><h2>{secret ? "密钥已创建" : "创建租户密钥"}</h2><p>{secret ? "可在租户密钥菜单中随时查看、复制或轮换。" : "为租户设置独立凭据和用量边界。"}</p></div><button className="icon-button" title="关闭" onClick={onClose}><X size={19} /></button></div>{secret ? <div className="secret-result"><label>租户 API Key</label><div><code>{secret}</code><button className="icon-button" title="复制密钥" onClick={async () => { await navigator.clipboard.writeText(secret); setCopied(true); }} >{copied ? <Check size={18} /> : <Clipboard size={18} />}</button></div><button className="primary full" onClick={onClose}>完成</button></div> : <form onSubmit={submit} className="create-form"><div className="form-grid"><label>名称<input value={name} onChange={(e) => setName(e.target.value)} required placeholder="生产应用" /></label><label>租户 ID<input value={tenant} onChange={(e) => setTenant(e.target.value)} required placeholder="tenant-prod" /></label><label>每分钟请求<input type="number" min="0" value={rpm} onChange={(e) => setRpm(e.target.value)} placeholder="不限" /></label><label>最大并发<input type="number" min="0" value={concurrent} onChange={(e) => setConcurrent(e.target.value)} placeholder="不限" /></label><label>每日 Token<input type="number" min="0" value={tokens} onChange={(e) => setTokens(e.target.value)} placeholder="不限" /></label><label>每日费用（元）<input type="number" min="0" step="0.01" value={cost} onChange={(e) => setCost(e.target.value)} placeholder="不限" /></label><label>每月预算（元）<input type="number" min="0" step="0.01" value={monthlyCost} onChange={(e) => setMonthlyCost(e.target.value)} placeholder="不限，达到后拒绝请求" /></label><label>可用模型<input value={allowedModels} onChange={(e) => setAllowedModels(e.target.value)} placeholder="留空表示全部模型" /></label></div><div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={busy}>{busy ? "正在创建" : "创建"}</button></div></form>}</div></div>;
}
